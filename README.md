...
- **项目级 Git Hook**：安装 post-commit hook，检测代码变更后提示更新文档（适用于项目中使用）
...
如果你想把项目代码作为知识库素材源，可以在项目中安装 Git Hook：

```bash
# 在项目根目录运行
bash ~/.claude/skills/llm-wiki/scripts/install-git-hook.sh
```

这会：
1. 安装 `.git/hooks/post-commit` hook
2. 创建 `.llm-wiki.yaml` 配置文件

安装后，每次提交代码时，如果变更文件数达到阈值，会输出提示建议运行 wiki digest。

卸载：

```bash
bash ~/.claude/skills/llm-wiki/scripts/install-git-hook.sh --uninstall
```
